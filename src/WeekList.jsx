...
export default class  WeekList extends Component{
    ...
    render()
    {
        ...
        const Buttons=(contractjson.map(row=>
            <Button id={row.contractid} color="link" key={row.contractid} onClick={(event)=>this.updateContract(event)}>
                {(moment(parseInt(row.date)*1000).format('ddd, DD MMM YYYY, HH:SS')).toString()}
            </Button>
    ))

        return(<div>
                <Accordion.Title active={this.props.parentState.state.activeIndex===this.props.number} index={this.props.number} style={{color:'white'}}  onClick={this.handleClick}>
                <Icon name='dropdown' />
                  {this.props.title}
                </Accordion.Title>
